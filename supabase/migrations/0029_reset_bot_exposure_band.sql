-- Migration 0029 — reset_bot connaît exposure_band_observations
--
-- La 0028 crée `public.exposure_band_observations` avec :
--
--     decision_id bigint not null references public.decisions (id) on delete cascade
--
-- La règle que la 0025 a écrite noir sur blanc, après l'avoir apprise à ses dépens :
-- TOUTE table portant une FK vers `public.decisions` doit figurer DANS la liste du TRUNCATE
-- de `reset_bot` ET dans son `grant truncate`. PostgreSQL refuse de tronquer une table
-- référencée par une foreign key à moins que la table référençante soit tronquée dans la
-- MÊME instruction. Sans ce nom ici, `reset_bot` échouerait entièrement avec :
--
--     ERROR: cannot truncate a table referenced in a foreign key constraint
--     DETAIL: Table "exposure_band_observations" references "decisions".
--
-- Et comme tout `reset_bot` tient dans une transaction, l'échec surviendrait APRÈS le claim
-- du run-lock : le rollback le relâche, rien n'est purgé, rien n'est corrompu. Le défaut
-- serait une INDISPONIBILITÉ du reset, découverte au pire moment — le jour où quelqu'un veut
-- réellement repartir de zéro. La 0022 avait créé ce trou et il a survécu trois migrations.
--
-- Vérification faite table par table, pour ne pas refermer la moitié du trou : les tables qui
-- portent aujourd'hui une FK vers `public.decisions` sont `executions` (0003),
-- `scheduler_runs` (0006), `equity_snapshots` (0008), `decision_guard_events` (0019),
-- `transition_observations` (0022), `refused_intentions` (0026) et désormais
-- `exposure_band_observations` (0028). Les six premières sont déjà dans la liste ; la
-- septième est le seul ajout. `executions.intent_execution_id` (0005) est une auto-référence
-- couverte par sa propre présence, `position_state` (0017) et `market_data_incidents` (0024)
-- ne portent aucune FK.
--
-- ── Ce que ce choix signifie pour le pilote ────────────────────────────────────
--
-- Un `reset_bot` efface les observations de bande avec le reste de l'historique, et c'est
-- délibéré : il redéfinit le capital de départ et tronque les décisions dont ces lignes sont
-- la lecture. Des observations survivant aux décisions qu'elles commentent ne seraient pas
-- une archive, elles seraient des orphelines pointant un `decision_id` réattribué à un autre
-- cycle par la séquence d'identité. L'identité PERSISTANTE du pilote — l'instant
-- d'activation, l'equity initiale, le plus-haut, l'état du coupe-circuit — arrive avec la
-- dernière brique et posera sa propre question au reset ; elle ne vit pas dans cette table.
--
-- Le reste de `reset_bot` est INCHANGÉ et ré-énoncé verbatim depuis la 0026 — cette migration
