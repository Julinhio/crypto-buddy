...
console.log('\n§2 — OBSERVE CHANGES NOTHING (proof 1, and proof 5 by construction)');
{
  ...
  const observed = applyGate({ mode: 'observe', movements, judgement, clampedAllocation: clamped, previousApplied: previous });
  ...
  const enforced = applyGate({ mode: 'enforce', movements, judgement, clampedAllocation: clamped, previousApplied: previous });
  const backToObserve = applyGate({ mode: 'observe', movements, judgement, clampedAllocation: clamped, previousApplied: previous });
...
console.log('\n§3 — ENFORCE: applied_allocation KEEPS THE PREVIOUS VECTOR (proof 3.1)');
{
  ...
  const clamped = { BTC: 10, ETH: 15, USDT: 75 };
  const previous = { BTC: 5, ETH: 20, USDT: 75 };
  const out = applyGate({ mode: 'enforce', movements, judgement, clampedAllocation: clamped, previousApplied: previous });
  ...
  const cold = applyGate({ mode: 'enforce', movements, judgement, clampedAllocation: clamped, previousApplied: null });
