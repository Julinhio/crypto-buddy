...
export function applyGate(params: {
  ...
  const { mode, movements, judgement, clampedAllocation, previousApplied } = params;

  // OBSERVE — the layer blocks nothing. Returned before anything is inspected so the
  // no-op is structural rather than a conclusion the code happens to reach.
  if (mode === 'observe') {
    return {
      movements,
  ...
  // Which legs survive: the deterministic exits, and only those. The judgement already
  // marked them, so this cannot disagree with what the journal records.
  ...
  return {
    ...
    reason:
      ...
      `deterministic exit(s) kept; applied_allocation holds the previous vector and no drift ` +
      `rebalancing is generated toward it`,
  };
}
