 ...
 *      rejects a perfectly correct response;
 *   3. worse, that stale reference is a standing instruction to hold a position the stop
 *      just closed — which can drive a buy back into it, violating the stop contract's
 *      explicit "no automatic re-entry" clause.
 *
 * So the weight goes to 0 and lands in the RESERVE, which is where the proceeds of the
 * exit actually went. Pure, and a no-op when no stop fired — which is every cycle in
 * `observe`, since `stopExits` is empty there.
 */
export function zeroOutStopped(
...
export function applyGate(params: {
  ...
  /** The cash side, where a stopped line's freed weight lands. */
  reserveAsset: string;
}): GateOutcome {
  const { mode, movements, judgement, stopExits, clampedAllocation, previousApplied, reserveAsset } = params;
  ...
  // Whatever base allocation we end up storing, a stopped line is FLAT in it. Applied to
  // both branches below — a stop can fire on an accepted cycle just as easily as on a
  // refused one, and the reference must describe the real book either way.
  const settle = (base: Record<string, number>): Record<string, number> =>
    zeroOutStopped(base, stopAssets, reserveAsset);
  ...
  if (!refused) {
    return {
      ...
      // THE ACCEPTED-CYCLE CASE, and the one the review caught. A stop can fire while the
      // strategic vector is perfectly fine — the model obeys and proposes nothing on the
      // frozen line. Storing the clamped proposal here would leave a POSITIVE weight on a
      // line the stop just emptied, and that value becomes the guard's reference next
      // cycle. See `zeroOutStopped`.
      appliedAllocation: settle(clampedAllocation),
      refused: false,
      droppedLegs: [],
      reason:
        stopExits.length > 0
          ? `${stopExits.length} peak-stop exit(s) generated by the code; the model's vector passes ` +
            `otherwise, and applied_allocation puts the stopped line(s) flat`
          : '',
    };
  }
  ...
  return {
    ...
    appliedAllocation: settle(previousApplied ?? clampedAllocation),
    refused: true,
    droppedLegs: dropped,
    reason:
      `${judgement.reason}${
