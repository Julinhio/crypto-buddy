...
export function buildSystemPrompt(cfg: AppConfig = config): string {
  ...
  return [
    ...
    'THE ORDER OF THE FIELDS IS PART OF THE CONTRACT: you reason FIRST, and the decision',
    'follows from the reasoning you have just written. Do not decide and then justify.',
    ...
    '  target you just wrote: "hold" means that target is unchanged from the last one.',
    ...
    '  in plain words, no numbers-heavy detail. It describes the target ABOVE it, so it',
    '  must not announce a move that target does not make. WRITE IT IN FRENCH — this field',
    '  ONLY; what_changed and reasoning stay in English. Non-empty even on a hold.',
